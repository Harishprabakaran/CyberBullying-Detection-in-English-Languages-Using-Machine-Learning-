import socket                       #Establishing client server connection
from _thread import *               #attend time run all the function
import sys
from collections import defaultdict as df
import pickle   #file path import
from sklearn.feature_extraction.text import TfidfVectorizer, TfidfTransformer
import time

# print(msg)
model = pickle.load(open(r"C:\Users\selvi\Desktop\Harish ECE\CyberBullying-Detection-in-English-Languages-Using-Machine-Learning-\Safe_Chat\LinearSVC.pkl", 'rb'))
...
class Server:
    ...
    def prettyPrinter(self,data_1):
        ...
        my_file.close()
        tfidf_vector =  TfidfVectorizer(stop_words = content_list, lowercase = True,vocabulary=pickle.load(open(r"C:\Users\selvi\Desktop\Harish ECE\CyberBullying-Detection-in-English-Languages-Using-Machine-Learning-\Safe_Chat\tfidf_vector_vocabulary.pkl", "rb")))
    ...
    def broadcast(self, message_to_send, connection, room_id,pred):
        for client in self.rooms[room_id]:
            if client != connection:
                try:
                    if pred==0:                                           
                        client.send(message_to_send.encode())             #positive message
                    else :
